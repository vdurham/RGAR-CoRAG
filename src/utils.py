...
# Original source: [https://github.com/Teddy-XiongGZ/MedRAG/blob/main/src/utils.py]  
# we add funtion retrieve_multi
...
class CustomizeSentenceTransformer(SentenceTransformer): # change the default pooling "MEAN" to "CLS"
    ...
    def _load_auto_model(self, model_name_or_path, *args, **kwargs):
        ...
        """
        if model_name_or_path in self._cached_models:
        ...
        self._cached_models[model_name_or_path] = [transformer_model, pooling_model]
...
class RetrievalSystem:
    ...
    def retrieve(self, question, k=32, rrf_k=100, id_only=False):
        ...
        return texts, scores
    def retrieve_multi(self, questions, k=32, rrf_k=100, id_only=False):
        """
        Given multiple questions, return the relevant snippets from the corpus with duplicate documents merged
        
        Args:
            questions: List[str] - List of query strings
            k: int - Number of documents to retrieve per query
            rrf_k: int - Parameter for reciprocal rank fusion
            id_only: bool - Whether to return only document IDs
            
        Returns:
            texts: List[dict] - List of retrieved documents with duplicates merged
        ...
        for question in questions:
            texts, scores = self.retrieve(question, k=k, rrf_k=rrf_k, id_only=id_only)
        ...
        for doc_id, doc_info in sorted_docs:
            ...
            # You could use different score aggregation strategies here:
            # Currently using max score across all queries
            merged_scores.append(doc_info['max_score'])
            
            # Add query score information to document if needed
            if not id_only:
